...
Firstly, a file path is setup so that RAT knows where to instal itself, in this case it's `%APPDATA%`, as shown with the `InstallFolder` varaible in `Client.Settings`.
